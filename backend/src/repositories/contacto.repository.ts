...
class ContactoRepository {
    ...
    async update(id_contacto: number, data: Partial<{ nombre: string; telefono: string; parentesco: string }>) {
        await pool.query(
            ...
            WHERE id_contacto = ?`,
            [data.nombre ?? null, data.telefono ?? null, data.parentesco ?? null, id_contacto]
        );
        return this.findById(id_contacto);
    }

    async delete(id_contacto: number) {
        await pool.query(`DELETE FROM contactoemergencia WHERE id_contacto = ?`, [id_contacto]);
    }
}
