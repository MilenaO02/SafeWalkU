...
export type ReportRegistryFilter = "ACTIVOS" | "ARCHIVADOS" | "TODOS";

class ReportRepository {
    async findAll(userId?: number, registryFilter: ReportRegistryFilter = "ACTIVOS") {
    ...
    async delete(id: number) {
        const [result] = await pool.query<ResultSetHeader>(
            "UPDATE reporte SET estado_registro = 'INACTIVO' WHERE id_reporte = ? AND estado_registro = 'ACTIVO'",
            [id]
        );
    ...
    async restore(id: number) {
        const [result] = await pool.query<ResultSetHeader>(
            "UPDATE reporte SET estado_registro = 'ACTIVO' WHERE id_reporte = ? AND estado_registro = 'INACTIVO'",
            [id]
        );
        return result.affectedRows;
    }
    ...
    async findActiveReportsByCity(ciudad: string) {
        const [rows] = await pool.query<RowDataPacket[]>(`
