...
export default function EditorRutas() {
  ...
  const reset = () => { setForm(emptyForm); setPoints([]); setEditingId(null); setDrawing(false); setError(null); setIsModalOpen(false); };
  ...
  return <div className="space-y-5">
    <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">Selecciona los extremos y haz clic sobre el mapa siguiendo aceras, curvas y cruces.</p>
      </div>
      <button onClick={() => { reset(); setIsModalOpen(true); }} className="whitespace-nowrap rounded-xl bg-purple-900 px-5 py-2.5 text-sm font-bold text-white shadow hover:bg-purple-950">Nueva ruta</button>
    </header>

    {isModalOpen && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
        <div className="relative flex h-full max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-slate-50 shadow-2xl">
          <header className="flex items-center justify-between border-b border-slate-200 bg-white px-5 py-4">
            <h3 className="text-lg font-black text-purple-950">{editingId ? `Editar ruta #${editingId}` : 'Nueva ruta'}</h3>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            ...
            <div className="grid gap-5 xl:grid-cols-[380px_1fr]">
              <form onSubmit={save} className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <div className="mt-auto grid grid-cols-2 gap-2 pt-4"><button type="button" onClick={reset} className="min-h-11 rounded-xl border border-slate-200 text-xs font-bold hover:bg-slate-50">Cancelar</button><button disabled={status === 'saving'} className="min-h-11 rounded-xl bg-purple-900 text-xs font-bold text-white shadow disabled:opacity-50 hover:bg-purple-950">{status === 'saving' ? 'Guardando…' : 'Guardar ruta'}</button></div>
              </form>
              <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-3">
                <div className="h-[520px] min-h-[360px] xl:h-full">
                  <MapaInteractivo
                    ...
                    zoom={17}
                    polyline={points}
                    onClick={(coords) => drawing && setPoints((current) => [...current, [coords.lat, coords.lng]])}
