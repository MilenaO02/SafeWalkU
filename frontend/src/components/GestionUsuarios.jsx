...
export default function GestionUsuarios() {
  ...
  const filtered = useMemo(() => users.filter((item) => {
    ...
    return value.includes(search.trim().toLowerCase()) && (role === 'TODOS' || item.rol === role);
  }), [users, search, role]);

  const deactivate = async (item) => {
    if (item.id_usuario === currentUser?.id_usuario || !window.confirm(`¿Desactivar a ${item.nombre} ${item.apellido}?`)) return;
    setBusyId(item.id_usuario);
    try {
      await request(`/users/${item.id_usuario}`, { method: 'DELETE' });
      setUsers((items) => items.filter((candidate) => candidate.id_usuario !== item.id_usuario));
  ...
  return <div className="space-y-5">
    <div><h2 className="text-xl md:text-2xl font-black text-purple-950">Gestión de usuarios</h2><p className="mt-1 text-xs text-slate-500">{users.length} usuarios activos obtenidos desde la API.</p></div>
    ...
    {status === 'ready' && <div className="grid gap-3">
      {filtered.map((item) => <article key={item.id_usuario} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center">
        ...
        <div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-900">{item.nombre} {item.apellido}</p><p className="truncate text-xs text-slate-500">{item.correo}</p><span className="mt-1 inline-block rounded-lg bg-slate-100 px-2 py-1 text-[10px] font-bold">{item.rol}</span></div>
        <button disabled={busyId === item.id_usuario || item.id_usuario === currentUser?.id_usuario} onClick={() => deactivate(item)} className="min-h-11 rounded-xl bg-red-50 px-4 text-xs font-bold text-red-700 disabled:cursor-not-allowed disabled:opacity-40">{busyId === item.id_usuario ? 'Desactivando…' : item.id_usuario === currentUser?.id_usuario ? 'Sesión actual' : 'Desactivar'}</button>
      </article>)}
