import React, { useEffect } from 'react';
import { MapContainer, TileLayer, Marker, Popup, Circle, useMap } from 'react-leaflet';
import L from 'leaflet';

import icon from 'leaflet/dist/images/marker-icon.png';
import iconShadow from 'leaflet/dist/images/marker-shadow.png';

// Solución al problema de íconos rotos en Leaflet con React
let DefaultIcon = L.icon({
  iconUrl: icon,
  shadowUrl: iconShadow,
  iconSize: [25, 41],
  iconAnchor: [12, 41],
  popupAnchor: [1, -34],
});
L.Marker.prototype.options.icon = DefaultIcon;

// Componente para manejar actualizaciones y animaciones de cámara del mapa
function ManejadorMapa({ centro, zoom = 17 }) {
  const map = useMap();
  
  useEffect(() => {
    if (map && centro) {
      map.invalidateSize();
      map.setView(centro, zoom, {
        animate: true,
        duration: 1.5
      });
    }
  }, [map, centro, zoom]);

  return null;
}

export default function MapaInteractivo({ 
  centro = [-3.9822, -79.2015], 
  zoom = 17, 
  markers = [
    { position: [-3.9822, -79.2015], title: "UIDE - Extensión Loja", desc: "Calle Agustín Carrión Palacios." }
  ],
  circle = null
}) {
  return (
    <div className="w-full h-full min-h-[300px] relative z-0">
      <MapContainer 
        center={centro} 
        zoom={zoom} 
        scrollWheelZoom={true} 
        className="w-full h-full absolute inset-0 rounded-3xl overflow-hidden shadow-inner"
      >
        <ManejadorMapa centro={centro} zoom={zoom} />

        <TileLayer
          attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
        />

        {/* Círculo de Rango o Alerta (ej: zona de riesgo) */}
        {circle && (
          <Circle 
            center={circle.center}
            pathOptions={{ 
              color: circle.color || '#ef4444', 
              fillColor: circle.color || '#ef4444', 
              fillOpacity: 0.15 
            }}
            radius={circle.radius || 90} 
          />
        )}

        {/* Mapeo dinámico de marcadores en el mapa */}
        {markers && markers.map((marker, idx) => (
          <Marker key={idx} position={marker.position}>
            <Popup>
              <div className="text-xs font-sans p-1">
                <p className="font-bold text-purple-950 text-sm mb-0.5">{marker.title}</p>
                <p className="text-slate-600 font-medium leading-relaxed">{marker.desc}</p>
              </div>
            </Popup>
          </Marker>
        ))}
      </MapContainer>
    </div>
  );
}