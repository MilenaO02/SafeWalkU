...
export default function AuthLayout() {
  ...
  return <div
    className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-cover bg-center bg-no-repeat p-4 font-sans"
  ...
  >
    ...
    <button onClick={toggleDarkMode} aria-label={isDarkMode ? 'Activar modo claro' : 'Activar modo oscuro'} className="absolute right-5 top-5 z-30 flex h-11 w-11 items-center justify-center rounded-full border border-white/30 bg-white/30 text-slate-700 shadow-sm backdrop-blur-md transition hover:bg-white/50 dark:bg-black/25 dark:text-slate-200 dark:hover:bg-black/40"><span className="material-symbols-outlined">{isDarkMode ? 'light_mode' : 'dark_mode'}</span></button>
    <main className="relative z-10 w-full max-w-[460px] animate-[fadeIn_0.45s_ease-out]"><Outlet /></main>
  </div>;
}
