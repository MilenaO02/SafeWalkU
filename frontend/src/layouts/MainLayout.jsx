...
export default function MainLayout() {
  ...
  // Configuración por defecto para el mapa (UIDE Loja)
  const defaultMapConfig = {
    centro: [-3.9822, -79.2015],
    zoom: 17,
    markers: [
      { 
        position: [-3.9822, -79.2015], 
        title: "UIDE - Extensión Loja", 
        desc: "Calle Agustín Carrión Palacios." 
      }
    ],
    circle: null
  };
  ...
  // Cada vez que cambia el layout principal, nos aseguramos de que haya un token de sesión.
  // Si no lo hay, redirigimos a login (en un entorno real). Para desarrollo, usaremos una sesión mock.
  useEffect(() => {
    const token = localStorage.getItem('token') || sessionStorage.getItem('token');
    if (!token) {
      // Si no hay token de sesión, creamos uno mock para no interrumpir el flujo de prueba
      localStorage.setItem('token', 'mock-token-session-safewalk');
      localStorage.setItem('user', JSON.stringify({ name: "Martín Estudiante", email: "martin@uide.edu.ec", rol: "estudiante" }));
    }
  }, []);
  ...
  const studentLinks = [
    ...
    { path: '/sos', label: 'SOS Emergencia', icon: 'emergency', highlight: true },
    { path: '/contactos', label: 'Apoyo', icon: 'contact_phone' },
    { path: '/perfil', label: 'Mi Perfil', icon: 'person' },
    { path: '/detalle-zona', label: 'Zona Bellavista', icon: 'info' }
  ];
  ...
  return (
    <MapContext.Provider value={{ mapConfig, setMapConfig, defaultMapConfig }}>
      <div className="flex flex-col h-screen w-screen bg-slate-50 overflow-hidden antialiased font-sans">
        ...
        {/* Área del Cuerpo: Sidebar izquierdo y Mapa derecho */}
        <main className="flex-1 flex overflow-hidden min-h-0 w-full relative">
          
          {/* Panel Lateral Izquierdo: Aquí rinde la vista activa del estudiante */}
          <aside className="w-full md:w-[32%] min-w-[360px] lg:w-[420px] bg-white border-r border-slate-200 overflow-y-auto custom-scrollbar flex flex-col flex-shrink-0 z-20 shadow-md relative rounded-tr-3xl">
            <div className="flex-1 p-6">
              <Outlet />
            </div>
          </aside>

          {/* Panel Derecho: Mapa Interactivo Leaflet persistente y animado */}
          <section className="flex-1 relative bg-slate-100 overflow-hidden h-full z-10">
            <MapaInteractivo 
