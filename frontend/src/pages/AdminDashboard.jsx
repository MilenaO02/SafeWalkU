...
export default function AdminDashboardContent() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-[#4A4A50] dark:bg-[#2B2B2F]"><h3 className="text-sm font-black text-slate-900 dark:text-white">Estados de reportes</h3><dl className="mt-4 space-y-3">{['PENDIENTE', 'VALIDADO', 'RECHAZADO', 'DUPLICADO', 'CANCELADO'].map((state) => <div key={state} className="flex items-center justify-between rounded-xl bg-slate-50 p-3 dark:bg-[#242428]"><dt className="text-xs font-bold text-slate-600 dark:text-slate-300">{formatLabel(state)}</dt><dd className="text-lg font-black text-purple-950 dark:text-purple-200">{statusCounts[state] || 0}</dd></div>)}</dl></div>
    </section>
